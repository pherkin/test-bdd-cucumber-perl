...
is($success_step{line},
    get_line_number($DIGEST_FEATURE_FILE, q{I've added "<data>" to the object}),
    "step line number");
...
like($failed_result->{error_message}, qr/
        got:[ ]'4'
        .*
        expected:[ ]'1'
...
like($skipped_result->{error_message},
...
is($todo_scenario{steps}[0]{name}, 'that we receive list of items from server');
my $todo_result = $todo_scenario{steps}[0]{result};
is($todo_result->{status}, 'pending', 'pending result status');
like($todo_result->{error_message}, qr/mock TODO message/,
...
like($missed_result->{error_message}, qr/No matching step definition for/,
